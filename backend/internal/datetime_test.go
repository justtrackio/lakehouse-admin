...
func TestDateTime_UnmarshalJSON_RFC3339(t *testing.T) {
	...
	if d.UTC().Format(time.RFC3339) != "2026-01-01T12:34:56Z" {
...
func TestDateTime_Value(t *testing.T) {
	...
	d = internal.DateTime{Time: time.Date(2026, 1, 2, 3, 4, 5, 0, time.UTC)}
	v, err = d.Value()
	if err != nil {
		t.Fatalf("value: %v", err)
	}
...
func TestDateTime_Scan(t *testing.T) {
	...
	if d.UTC().Format(time.RFC3339) != "2026-01-01T00:00:00Z" {
		t.Fatalf("unexpected time: %v", d.Time)
	}
}
