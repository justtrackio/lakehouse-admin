...
import (
	...
	"github.com/justtrackio/lakehouse-admin/internal/mocks"
...
type ModuleTasksSuite struct {
	suite.Suite
	module    *internal.ModuleTasks
	claimer   *mocks.MockTaskClaimer
	executor  *mocks.MockMaintenanceExecutor
	refresher *mocks.MockSnapshotRefresher
	sqlDB     *sql.DB
...
func (s *ModuleTasksSuite) SetupTest() {
	...
	s.claimer = mocks.NewMockTaskClaimer(s.T())
	s.executor = mocks.NewMockMaintenanceExecutor(s.T())
	s.refresher = mocks.NewMockSnapshotRefresher(s.T())
	...
	s.module = internal.NewModuleTasksForTest(
		logger,
		s.claimer,
		s.executor,
		s.refresher,
...
func (s *ModuleTasksSuite) TestProcessTask_ExpireSnapshots() {
	...
	s.executor.EXPECT().
		...
		Return(&internal.ExpireSnapshotsResult{
			Table:                "test_table",
			RetentionDays:        7,
			RetainLast:           5,
...
func (s *ModuleTasksSuite) TestProcessTask_RemoveOrphanFiles() {
	...
	s.executor.EXPECT().
		...
		Return(&internal.RemoveOrphanFilesResult{
			Table:         "test_table",
			RetentionDays: 14,
...
func (s *ModuleTasksSuite) TestProcessTask_Optimize() {
	...
	s.executor.EXPECT().
		...
		Return(&internal.OptimizeResult{
			Table:               "test_table",
...
func (s *ModuleTasksSuite) TestSetWorkerCount() {
	s.Equal(2, s.module.GetWorkerCount())

	s.module.SetWorkerCount(5)
	s.Equal(5, s.module.GetWorkerCount())

	// Test clamping to minimum of 1
	s.module.SetWorkerCount(0)
	s.Equal(1, s.module.GetWorkerCount())
...
func (s *ModuleTasksSuite) TestRun_ProcessesTask() {
	...
	s.claimer.EXPECT().
		...
		RunAndReturn(func(ctx context.Context) (*internal.Task, error) {
			callCount++
			if callCount == 1 {
				return task, nil
			}
