...
func (c *SparkClient) Call(ctx context.Context, query string, result any) error {
	...
	for _, row := range rows {
		...
		if err = slice.Append(elem); err != nil {
			return fmt.Errorf("failed to append to slice: %w", err)
		}
	}
...
func (c *SparkClient) QueryRowsPaged(ctx context.Context, query string) ([]map[string]any, error) {
	...
	for {
		if page, err = c.fetchPage(ctx, query, limit, offset); err != nil {
			return nil, err
		}
...
func (c *SparkClient) fetchPage(ctx context.Context, query string, limit, offset int) ([]map[string]any, error) {
	var err error
	var df sql.DataFrame
	var columns []string
	var rows []types.Row
